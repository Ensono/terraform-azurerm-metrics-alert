...
module "under_test" {
    ...
    "service1dlq_critical" = {
      criteria = {
        entity_name_include = {
          ...
          dimension_values   = distinct([for i, q in azurerm_servicebus_queue.default : substr(q.name, 0, length(q.name) - 2) if i < 5])
  ...
  action_group = {
    "key" = {
      email_receiver = {
        "key" = {
          email_address = "dusan.nitschneider@ensono.com"
